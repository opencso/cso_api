...
generate.sql <- function(){
    ...
    write.table(my, file=paste0("/home/dave/cso/api/output/",current_filename), col.names=FALSE, sep=",")
    ## create
    all_sql <- ""
    ...
    for(i in 1:length(set_ids)){
        if(is.factor(my[,i])){
            ...
            sql_string <- paste0(sql_string, "value real\n);\n")
                
        }

    }
    sql_string <- paste0(sql_string, "\\copy ", current_filename, "FROM '/home/dave/cso/api/output/", current_filename,".csv' DELIMITER ',' CSV;\n\n")
    all_sql <- paste0(all_sql, sql_string)
    cat(all_sql, file="all_sql.csv")
